...
采纳 [CR-20260820-001](command/change-requests/CR-20260820-001-命令多目录输入.md)。用户需求：新产品文档把命令拆到多个目录（多个分册），抽取时需选择多个命令源目录；其余角色/功能不变。

- `command/scripts/build_commands.py`：`--mml-dir` 改 `action="append"`（可重复传参，单次构建传入全部目录）——参见边两趟校验依赖**全量命令名集**，分次跑会丢边，故必须单次运行；多目录 rglob 并集按 resolved 去重（目录重叠不重复建），category_path 取包含该文件的第一个源目录相对路径。**单目录用法完全兼容**。
- 回归测试：`MultiMmlDirsTests`（双目录单跑，核心断言=分册一命令建出指向分册二命令的参见边）。
- 平台侧（graph-asset-platform）：构建器字节一致同步；`MineIn.dirs` 的 mml 角色归一化为列表（兼容单值）；run_mine 目录校验/参数拼装支持列表；前端抽取面板命令角色改多选（推荐+候选可多选、逐层浏览逐个加入）。
